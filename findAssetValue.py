#
# acctAnalysisSum.py -- Find the value of an asset or liability at a certain date
#
    ...
    try:
        ...
        if len(descendants) == 0:
            ...
            for subAcct in descendants:
                suba_name = subAcct.GetName()
                suba_bal = subAcct.GetBalanceAsOfDate(date(2018, 12, 31))
                suba_comm = subAcct.GetCommodity()
                print("{} balance on 2018-12-31 in {} = {}".format(suba_name, suba_comm.get_fullname(), suba_bal))
                suba_cad = subAcct.ConvertBalanceToCurrencyAsOfDate(suba_bal, suba_comm, CAD, date(2018, 12, 31))
                print("{} balance on 2018-12-31 in CAD = {}".format(suba_name, suba_cad))
